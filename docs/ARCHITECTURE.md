...
- 账号与会话由 NextAuth + GeniLink SSO 管理，工作区成员关系不在当前 Prisma schema 中建模
...
```
当前仓库的 Prisma schema 只定义内容与发布相关的核心表，没有单独的 `User`、`Workspace` 或 `WorkspaceMember` 表。
租户隔离主要通过业务层里的 `workspaceId` 和 `projectId` 约束实现，认证与会话则由 NextAuth + GeniLink SSO 提供。

ContentPiece ────< PlatformContent
     │            ├── ReviewComment
     │            ├── QualityHistory
     │            ├── ContentSchedule (1:1, 通过 `contentId` 唯一)
     │            ├── ContentQuality (1:1)
     │            └── BrandVoice? (可选)
     │
     └── brandId / createdByUserId / workspaceId 作为逻辑租户与审计字段

BrandVoice ────< ContentPiece
     │
     └── workspaceId / brandId / createdByUserId

AITemplate / GenieSource / PlatformApiConfig / Notification / PublishHistory
     └── 以 workspaceId 或业务主键进行隔离与查询
```
...
```typescript
{
  ...
  brandVoiceId?: string;
  status: string;              // draft/genie_draft/editing/review/...
...
```typescript
{
  ...
  description?: string;
  guidelines?: string;
  samples: string;             // JSON array / medium text
  createdByUserId: string;
}
```
